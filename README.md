...
大数组和固定宽度记录使用批量读写；法线校验与顶点、面角读取使用 Blender 批量接口。测量用现有 Typhoea Mod 的三个 Mesh，在 Blender 5.2 后台进程中导入从约 2.55 秒降到 1.21 秒，仅网格包导出从约 4.66 秒降到 2.68 秒。两次导出包逐文件完全相同。

另用 Blender 5.0.1 打开作者工程，对 body 和 cloth01 单线程导出：分别从 1.17 秒降到 0.91 秒、1.17 秒降到 0.85 秒，生成的 Mesh 二进制逐字节一致。作者工程不保存，测量输出写入临时目录。实际耗时会随网格大小、形态键、贴图、磁盘和 Blender 版本变化。
